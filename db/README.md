...
Каталог `migrations/pending/` этим циклом не применяется: там лежат миграции,
ждущие решения оператора (например, удаление модели анализа v1 — см.
`operations/runbooks/ANOMALY.md`). Номер у них уже следующий по порядку.
...
Сверка каталога объектов привилегий не проверяет: `tools/privileges.sql` снимает
права ролей отдельно, и сверять надо оба среза. Разница по правам должна сводиться
к грантам удалённых таблиц плюс явное расширение из `0021`.
