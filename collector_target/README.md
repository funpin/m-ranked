...
- Each changed batch emits `cache.invalidated` with bounded affected tags plus a
  ...
  Exact account replay creates neither another event nor another revision.
...
Unit tests live in `tests/test_target_collectors.py`. The real-schema suite in
`tests/test_target_collectors_postgres.py` requires
`MRANKED_TEST_POSTGRES_DSN` set to a collector-role DSN for a database with
the exact current schema contract. Fixture creation/cleanup uses
`MRANKED_TEST_POSTGRES_ADMIN_DSN`; it falls back to the first DSN only for a
privileged disposable test database. It verifies public
baseline/actual persistence, retry idempotency, monotonic forced-incomplete
merge, and live bounded reads from publication zero.
...
same protected root used by the API and backup verifier; each
