...
def persist_native_csv(connection, publication_id, month, snapshot_id, evidence):
    ...
    raw={
        '_mranked_reverse_sync': {
            'version':2,'legacy_table':'reaction_snapshots' if telegram else 'platform_snapshots',
            'publication_id':str(publication_id),'published_month':month.isoformat(),
            'snapshot_id':snapshot_id,'collected_at':row['collected_at'].isoformat(),
