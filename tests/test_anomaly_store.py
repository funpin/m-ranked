...
REMOVAL = Path(__file__).resolve().parents[1] / "db/migrations/pending/0037_remove_anomaly_analysis_v1.sql"
...
def test_removal_of_v1_waits_outside_the_applied_migrations():
    ...
    assert not list(root.glob("0037_*.sql")), "удаление v1 не должно применяться обычным прогоном"
