    ...
    try:
        ...
        public_events = admin.execute(
            """SELECT event.event_type, event.published_at
                ...
                WHERE event.dataset_revision_id=%s ORDER BY event.id""",
            (first.revision_id,),
        ).fetchall()
        assert public_events == [
            {"event_type": "cache.invalidated", "published_at": None},
            {"event_type": "source.account.updated", "published_at": None},
        ]
...
def test_real_postgres_account_transaction_is_idempotent_and_atomic(imported_publication_id: bool) -> None:
    ...
    try:
        ...
        # One ingestion emits a cache invalidation and a domain event exactly
        # once. The replayed batch above adds neither.
        assert admin.execute(
