...
class _ScriptedConnection:
    ...
    def execute(self, sql: str, params: Any = None) -> _Cursor:
        ...
        if "INSERT INTO ingest.metric_evidence_dictionary" in normalized:
            return _Cursor({"id": 7})
...
def test_metric_evidence_dictionary_is_cached_per_process() -> None:
    connection = _ScriptedConnection()
    repository = PostgresCollectorRepository(connection_factory=lambda: connection)
    evidence = {"views": {"source_field": "views", "quality": "exact", "flags": {}}}
    assert repository._metric_evidence_id(evidence) == 7
    assert repository._metric_evidence_id(evidence) == 7
    assert sum("INSERT INTO ingest.metric_evidence_dictionary" in statement
               for statement, _ in connection.calls) == 1
