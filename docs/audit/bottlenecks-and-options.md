...
| P1 | Rutube perpetually overruns | 300 s global interval против cycle p50 589/p95 750; 91% partial | repository fix: platform-specific 3600 s Rutube interval, 300 s TG/VK/MAX; rollout pending | убрать бесконечный catch-up и CPU churn; freshness станет предсказуемой | env/release rollback; production remains unchanged until approved rollout |
| P1 | MAX request can hang above the DB layer | PyMax await had no local deadline; watchdog could only restart the process later | repository fix: 30 s connect/request timeout, close and invalidate client on failure; rollout pending | bound one provider call and recover in-process | configurable timeout; release rollback |
| P2 | Collector writes grow round trips with publications/reactions | publication, identity, latest snapshot, reaction, presence and legacy export paths executed per row | repository fix: JSONB recordsets and set-based writes per account transaction; rollout pending | bounded SQL round trips per account batch | same tables/triggers/transaction; release rollback |
