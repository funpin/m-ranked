...
показатели из общей схемы `analytics`. Работающий legacy-релиз пока использует SQLite.
...
Реализация Next.js / Spring / PostgreSQL / Redis находится в этом монорепозитории.
Текущая схема — Flyway V1–V29; production пока остаётся на legacy, Writer Gate W
закрыт. Статус маршрутов и проверок:
