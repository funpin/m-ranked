...
- `RUTUBE_PUBLIC_API_ENABLED=true` — включает официальный публичный фид
  видео и просмотров; токен для этого режима не требуется;
...
`chat_id` каждого канала. Rutube использует официальный публичный JSON-фид:
он даёт просмотры без токена, а недоступные likes/comments сохраняются как
`NULL`, не как ноль. Полный перечень секретов и прав — в
