...
Актуальная версия исходников находится в **`main`**: 7 сентября 2026 года
в неё перенесены изменения `alpha` и выполнена очистка сгенерированных
артефактов. Этот README описывает текущий `main`.

Интерфейс — **Next.js (`frontend/`) + Spring Boot (`backend/`)**.
Приложение хранит UTC-замеры всех площадок в PostgreSQL и читает рассчитанные
...
Python-код сборщиков и миграции данных сохранён. Старый Python-интерфейс,
его шаблоны и команды `web`/`run` удалены из текущего дерева.
Для сравнительных проверок используется
[отдельная эталонная версия](migration/legacy-reference.md).

[Локальный Docker-стенд](infra/local/README.md) открывается на
http://localhost:3000. Состояние серверного переноса описано отдельно ниже:
обновление `main` само по себе не развёртывает приложение на сервере.
...
## Текущий стек и серверный перенос

Стек текущих исходников и окружения проверок:

| Компонент | Версия |
| --- | --- |
| Интерфейс | Next.js 16.3.3, React 19.2.0, TypeScript |
| Frontend toolchain | Node 24, pnpm 11.19.0 |
| API | Spring Boot 4.0.8, Java 21 |
| База и кеш | PostgreSQL 18.6, Redis 8.10.0 |
| CI и серверный импортёр | Python 3.13 |
| Схема и перенос данных | Flyway V1–V30, bridge 1.3.1 |

**Серверный статус ниже зафиксирован в отчётах от 6 сентября 2026 года.**
На момент проверки PostgreSQL был установлен, выполнялся начальный перенос
проверенного снимка SQLite, главная страница показывала прогресс, а старые
сборщики продолжали писать в SQLite. Завершение переноса, догрузка новых
записей и переключение приложения в этих отчётах ещё не подтверждены;
Writer Gate W был закрыт. Это датированная запись, а не текущая телеметрия.
Подробности:
...
[исторический отчёт ревью V29](migration/reports/review-v29-20260906.md),
...
### Артефакты проверок и размер репозитория

7 сентября из текущего дерева удалены 26 251 сгенерированный файл общим
несжатым объёмом 4,71 ГБ: HTML/PNG-снимки, подробные дампы браузерных
проверок, временные отчёты прогресса, логи, JUnit-выгрузки и тестовые
Parquet-архивы. Исходники, тесты, ресурсы приложения, сводные отчёты и
документы приёмки сохранены.

Новые прогоны в `frontend/evidence/`, `migration/reports/` и
`operations/**/evidence/` исключены через `.gitignore`; сырые результаты
остаются локально или загружаются как CI-артефакты. Сохранённые отчёты
помечены в `.gitattributes` как сгенерированные и исключены из статистики
языков GitHub.

История Git не переписывалась: старые коммиты по-прежнему содержат архивы,
поэтому очистка текущего дерева не уменьшает объём полной истории.
Пути и контрольные суммы внутри исторических отчётов относятся к исходным
прогонам; всех сырых файлов в свежем checkout больше нет.
[Правила хранения и восстановление исторических артефактов](docs/validation-artifacts.md).
...
### Модель legacy-релиза для переноса и отката
...
Next.js/Spring. Этот путь используется при работе с исходной SQLite-базой
до подтверждённого переключения серверных сборщиков.
...
Конфигурации запуска старого веб-интерфейса больше не входят в `main`.
