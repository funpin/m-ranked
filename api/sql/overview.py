"""Обзор: карточки учреждений с агрегатами за период.

Счётчики публикаций берутся из analytics.publication_latest — витрины, которую
коллектор обновляет при записи. Прежняя реализация вызывала
analytics.publication_snapshot_slice(publication_id, month) через LATERAL на
каждую публикацию периода; функция объявлена SECURITY DEFINER с заданным
search_path, планировщик не может её заинлайнить, и на 18 957 публикациях это
49 385 719 буферов. Чтение витрины — 28 782 строки.

Представление analytics.usable_publication_snapshot здесь неприменимо: его
коррелированный NOT EXISTS по successor не отсекает партиции.
"""
from __future__ import annotations

OVERVIEW = """
WITH params AS (
    SELECT CAST(%(as_of)s AS timestamptz) AS as_of,
     ...
     CROSS JOIN params
      ...
      LEFT JOIN LATERAL (
          SELECT observation.started_at,
           ...
           ORDER BY observation.started_at DESC, observation.id DESC LIMIT 1) result ON true
), dimensions AS (
    SELECT 'telegram'::text AS scope_platform, account.id AS entity_id,
     ...
     WHERE %(platform)s='telegram' AND account.platform='telegram'
       AND account.enabled AND account.channel_legacy_id IS NOT NULL
    UNION ALL
    SELECT %(platform)s, institution.id, 'institutions', alias.legacy_id, alias.legacy_route, institution.id
     ...
     WHERE %(platform)s IN ('all','vk','max','rutube')
    ...
    SELECT scope_platform, entity_id, count(*)::integer AS account_count,
           ...
           sum(subscriber_count)::bigint AS subscriber_count,
    ...
    SELECT dimension.scope_platform AS platform, %(period)s AS period_key, dimension.entity_type,
           ...
           CASE WHEN coalesce(summary.account_count,0)=0 THEN 'no_account'
                ...
                WHEN summary.last_checked_at IS NOT NULL THEN 'polling'
           ...
           coalesce(metrics.shares_samples,0) AS shares_samples,
           metrics.publication_count AS sample_denominator,
           params.as_of
    ...
    SELECT * FROM filtered
     WHERE CAST(%(after_id)s AS uuid) IS NULL
        OR page_position>(SELECT page_position FROM filtered WHERE entity_id=CAST(%(after_id)s AS uuid))
