...
const PUBLICATION_SORT_OPTIONS = [
  ["erv", "ERV"], ["views", "Просмотры"], ["reactions", "Реакции / лайки"],
  ["interactions", "Взаимодействия"], ["published_at", "Дата публикации"],
] as const;
...
export default async function StatisticsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <>
    <PageHeader
      title="Статистика публикаций"
      titleNote={<MethodNote title="Как считается статистика"><p>Период отбирает публикации по дате выхода. Для них используются последние накопленные счётчики. Взаимодействия — реакции или лайки, комментарии и репосты; структурно неподдерживаемый компонент равен нулю только в этой формуле. ERV = взаимодействия / просмотры × 100%. Если поддерживаемого счётчика нет или просмотры равны нулю, значение неизвестно.</p></MethodNote>}
    ...
    <StatisticsFilterForm key={selectionKey} action="/statistics" method="get" aria-label="Фильтры статистики" className="mb-5 grid gap-3 rounded-xl border bg-card p-3 lg:grid-cols-[auto_auto_minmax(14rem,1fr)_auto_auto] lg:items-end">
      <NativeSegments name="platform" legend="Площадка" value={query.platform} labelled={false}
      ...
      <label className="grid gap-1 text-xs font-medium text-muted-foreground"><span>Период</span><NativeSelect name="period" defaultValue={query.period} aria-label="Период статистики">{Object.entries(PERIOD_LABELS).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</NativeSelect></label>
      <label className="grid gap-1 text-xs font-medium text-muted-foreground"><span>Поиск</span><span className="flex gap-1"><NativeInput name="q" defaultValue={query.q} maxLength={200} placeholder="Вуз, аккаунт, ID или URL" aria-label="Поиск публикаций" />{query.q ? <Link className="inline-flex size-9 shrink-0 items-center justify-center rounded-md border" href={queryHref("/statistics", { ...statisticsHrefQuery(query), q: undefined })} aria-label="Очистить поиск" prefetch={false}><X className="size-4" aria-hidden="true" /></Link> : null}</span></label>
      <label className="grid gap-1 text-xs font-medium text-muted-foreground"><span>Сортировка</span>{query.view === "publications"
        ...
        : <NativeSelect name="entity_sort" defaultValue={query.entitySort} aria-label="Сортировка вузов">{ENTITY_SORT_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</NativeSelect>}</label>
      <label className="grid gap-1 text-xs font-medium text-muted-foreground"><span>Направление</span>{query.view === "publications"
        ...
        : <NativeSelect name="entity_direction" defaultValue={query.entityDirection} aria-label="Направление сортировки вузов"><option value="desc">По убыванию</option><option value="asc">По возрастанию</option></NativeSelect>}</label>
      ...
      <NativeButton type="submit" className="lg:self-end" aria-label="Найти"><Search className="size-4" aria-hidden="true" /><span>Найти</span></NativeButton>
