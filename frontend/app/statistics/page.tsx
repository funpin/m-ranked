...
export default async function StatisticsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return <>
    <PageHeader
      title="Статистика публикаций"
      titleNote={<MethodNote title="Как считается статистика"><p>Период отбирает публикации по дате выхода. Для них используются последние накопленные счётчики. Взаимодействия — реакции или лайки, комментарии и репосты; отсутствующий или неподдерживаемый компонент даёт нулевой вклад только в этой формуле. ERV = взаимодействия / просмотры × 100%. У вуза ERV взвешен по просмотрам публикаций с ненулевыми просмотрами. Если положительного числа просмотров нет, ERV вычислить нельзя.</p></MethodNote>}
