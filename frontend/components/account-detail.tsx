...
function MetricWithGrowth({ value, quality, growth, label }: { value: number | null; quality: string | null; growth?: number | null; label: string }) {
  return <TableCell title={quality ?? undefined}><span className="flex items-center gap-2 whitespace-nowrap"><span>{legacyNumber(value)}</span><DeltaBadge value={growth} compact={false} label={`${label} за выбранные сутки`} /></span></TableCell>;
}
