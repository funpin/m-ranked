...
export function mountHeroScene(canvas: HTMLCanvasElement, host: HTMLElement, options: SceneOptions): SceneHandle {
  ...
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, options.compact ? 1.5 : 1.75));
  ...
  const data = sceneCurves({ count: options.compact ? 18 : 26 });
  ...
  for (const [index, item] of data.entries()) {
    ...
    const tube = new TubeGeometry(curve, TUBE_SEGMENTS, item.lead ? 0.02 : 0.009, TUBE_SIDES, false);
    const tubeMaterial = new MeshBasicMaterial({ transparent: true, opacity: item.lead ? 1 : 0.5, depthWrite: false });
    ...
    const wallMaterial = new MeshBasicMaterial({
      vertexColors: true, transparent: true, opacity: item.lead ? 0.2 : 0.07, side: DoubleSide, depthWrite: false,
    });
    ...
    const points = item.points.filter((_, step) => step % 2 === 1).map(([x, y, z]) => new Vector3(x, y, z));
  ...
  function setPalette(palette: ScenePalette) {
    ...
    grid = new GridHelper(8, 24, new Color(palette.grid), new Color(palette.grid));
    const gridMaterial = grid.material as Material;
    gridMaterial.transparent = true;
    gridMaterial.opacity = palette.dark ? 0.35 : 0.6;
    model.add(grid);
    render();
  }
  ...
  function resize() {
    ...
    const shift = options.compact ? 0 : clamp((width / height - 1) * 0.34, 0, 0.3);
  ...
  function place(time: number) {
    ...
    const distance = lerp(options.compact ? 13 : 12, 10, progress);
