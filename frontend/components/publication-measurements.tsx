...
type Metric = typeof definitions[number];
function noun(metric:Metric,platform:string) {return ({reactions:platform === "vk" || platform === "rutube" ? "лайков" : "реакций",views:"просмотров",comments:"комментариев",shares:"репостов"})[metric.key];}
function metricLabel(metric:Metric,platform:string) {return metric.key === "reactions" && platform === "vk" ? "Лайки" : metric.label;}
...
function MetricChart({ rows, metrics, delta, selectedId, onSelect, platform }: {
  rows: HistorySnapshot[]; metrics: Metric[]; delta: boolean; selectedId?: string;
  onSelect: (id: string) => void; platform:string;
}) {
  ...
  useEffect(() => {
    ...
    void import("@/lib/chart-history").then(({ default: ChartJS }) => {
      ...
      applyTheme();
      const value=(metric:Metric,index:number) => {
        const row=rows[index]!;
        if(!delta) return row[metric.key].value;
        if(!index) return row[metric.delta];
        const previous=rows[index-1]![metric.key].value,current=row[metric.key].value;
        return current===null || previous===null ? null : current-previous;
      };
      const commonTitle=metrics.map((metric)=>metricLabel(metric,platform)).join(" и ");
      const axisTitle=platform === "telegram" ? delta ? "Прирост реакций и просмотров" : "Реакций и просмотров" : metrics.length>2 ? delta ? "Прирост метрик" : "Метрики" : delta ? `Прирост: ${commonTitle.toLowerCase()}` : commonTitle;
      const chart = new ChartJS<"line"|"bar", (number|null)[]>(canvas.current, {
        type:delta ? "bar" : "line", data:{ labels:rows.map((row) => [shortDate(row.observedAt),row.synthetic ? "момент публикации" : `через ${duration(row.ageHours*3600)}`]), datasets:metrics.map((metric) => ({ label:`${delta ? "Прирост" : "Всего"} ${noun(metric,platform)}`,
          data:rows.map((_row,index) => value(metric,index)), borderColor:metric.color, borderWidth:delta ? 1 : 3, backgroundColor:delta ? rows.map((_row,index) => (value(metric,index) ?? 0) < 0 ? "#c13b4f" : `${metric.color}${platform === "telegram" && metric.key === "views" ? "bb" : "cc"}`) : `${metric.color}${platform === "telegram" && metric.key === "views" ? "1f" : "22"}`,fill:!delta,
          ...
          plugins:{legend:{display:false},tooltip:{callbacks:platform === "telegram" ? {afterBody:(items)=>{const row=rows[items[0]?.dataIndex??-1];return !delta&&row&&row.views.value!==null&&row.views.value>0&&row.reactions.value!==null?`Реакции / просмотры: ${(row.reactions.value*100/row.views.value).toFixed(2)}%`:"";}} : {afterTitle:(items) => rows[items[0]?.dataIndex ?? -1]?.synthetic ? "Момент публикации · синтетическая точка" : ""}}},
          ...
          }, onClick:(_event, elements) => { const row = rows[elements[0]?.index ?? -1]; if (row) onSelect(row.snapshotId); },
        },
      });
  ...
  }, [rows,metrics,hidden,scale,delta,onSelect,platform]);
  ...
  function keyboard(key?: string) {
    ...
    setTooltip(`${legacyDate(row.observedAt)} · ${metrics.filter((metric) => !hidden.has(metric.key)).map((metric) => `${metric.label}: ${legacyNumber(delta ? row[metric.delta] : row[metric.key].value)}`).join(" · ")}`);
  }
  ...
  return <>
    ...
    <div className="chart"><canvas ref={canvas} role="img" tabIndex={0} data-chart-ready={ready} aria-label={delta ? "Прирост между замерами" : "Накопление показателей"} aria-describedby={`${chartId}-instructions ${chartId}-tooltip`}
      onFocus={() => keyboard()} onKeyDown={(event) => { if(event.key === "Escape") { closeTooltip(); }else if(["ArrowLeft","ArrowRight","Home","End"].includes(event.key)) { event.preventDefault();keyboard(event.key); } }} onBlur={closeTooltip}>Все значения доступны в таблице истории замеров.</canvas></div>
    <p id={`${chartId}-instructions`} className="sr-only">Стрелки влево и вправо выбирают замер; Home и End — первый и последний. Escape закрывает подсказку.</p>
...
export function PublicationMeasurements({ rows, platform, historyLimit }: { rows: HistorySnapshot[];platform:Exclude<Platform,"all">;historyLimit:number }) {
  ...
  const metrics = useMemo(() => definitions.filter((metric) => telegram ? ["reactions","views"].includes(metric.key) : rows.some((row) => row[metric.key].value !== null)),[rows,telegram]);
  ...
  const tableRows = rows.slice(-Math.max(1,historyLimit));
  ...
  const tableMetrics = telegram ? definitions.filter((metric) => metric.key !== "shares") : metrics;
  ...
  return <>
    <div className="grid"><div className="panel"><h2>Накопление {phrase}</h2><p className="panel-note">{telegram ? "Обе линии" : "Все линии"} построены по одним и тем же замерам. В режиме 1:1 используется общая шкала; «Авто» накладывает кривые с независимыми шкалами для сравнения их формы.</p><MetricChart rows={displayed} metrics={metrics} delta={false} selectedId={selectedId} onSelect={setSelectedId} platform={platform} /></div>
      <div className="panel"><h2>Прирост между замерами</h2><p className="panel-note">Сколько новых {phrase} появилось после предыдущего опроса. В режиме 1:1 используется общая шкала; «Авто» показывает показатели на независимых шкалах.</p><MetricChart rows={displayed} metrics={metrics} delta selectedId={selectedId} onSelect={setSelectedId} platform={platform} /></div></div>
    ...
    <div className="panel mt measurement-history"><h2>История замеров</h2>
      {telegram && rows.length > historyLimit ? <p className="panel-note">Показаны последние {tableRows.length} из {rows.length} замеров · <a href={`?history_limit=${Math.min(1000,rows.length)}`}>показать всю историю</a></p> : telegram && rows.length > 100 ? <p className="panel-note">Показаны все {rows.length} замеров · <a href="?history_limit=100">свернуть историю</a></p> : null}
      {rows.length ? <div className="measurement-history-scroll"><table className="snapshot-history-table"><thead><tr>{[
        ...
        ...tableMetrics.flatMap((metric) => [[metric.icon,metric.label],[`Δ${metric.icon}`,`Дельта ${metric.label.toLowerCase()}`],...(telegram && metric.key === "reactions" ? [["👥","Минимум людей"]] : [])]),
        ...(telegram || platform === "max" ? [["😀","Реакции по типам"],["Δ😀","Дельта реакций по типам"]] : []),
        ...
        {tableRows.map((row,index) => {
          ...
          return <tr id={`snapshot-${row.snapshotId}`} key={row.snapshotId} className={`snapshot-row${row.synthetic ? " synthetic-row" : ""}${selectedId === row.snapshotId ? " snapshot-highlight" : ""}`} title={`${row.quality}${row.intervalUncertain ? " · интервал неопределён" : ""}`}><td><button type="button" className="snapshot-jump" title="Показать эту точку на графике" onClick={() => jump(row.snapshotId)}>{legacyDate(row.observedAt)}</button></td><td>{signedDuration(elapsed)}</td><td>{row.synthetic ? "момент публикации" : duration(row.ageHours*3600)}</td>
            ...
            {telegram || platform === "max" ? <><td className="reaction-cell"><Breakdown value={historyReactionEntries(row)} /></td><td className="reaction-cell"><Breakdown value={historyReactionEntries(row,true)} delta /></td></> : null}</tr>;
        })}
