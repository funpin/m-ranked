...
def build(destination: Path) -> dict:
    ...
    published = FIXTURE_ANCHOR - timedelta(days=16)
    ...
    for index in range(1, 206):
        ...
        for hour in [0, 1, 24, 48, 72, 168, 336, 360, 381, 383, 384]:
    ...
    return {"path": str(destination.resolve()), "sha256": hashlib.sha256(destination.read_bytes()).hexdigest(), "clock": FIXTURE_ANCHOR.isoformat(), "counts": counts,
            "coverage": ["telegram", "vk", "max", "rutube", "null", "zero", "negative", "reactions", "comments", "shares", "subscribers", "deleted", "recovered", "album", "joint", "repost", "custom_emoji", "incomplete", "uncertain", "synthetic", "official_rating", "rating_over_200"]}
