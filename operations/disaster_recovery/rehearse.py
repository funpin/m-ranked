...
class Rehearsal:
    def __init__(self, source_container: str, source_database: str, report_dir: Path):
        ...
        self.passwords = {key: secrets.token_hex(24) for key in (
            ...
            'BACKUP_DB_PASSWORD','MAINTENANCE_DB_PASSWORD',
            'ANALYTICS_WORKER_DB_PASSWORD','OUTBOX_WORKER_DB_PASSWORD')}
    ...
    def execute(self):
        ...
        with tempfile.TemporaryDirectory(prefix='mranked-dr-secret-') as secret_dir:
            ...
            try:
                ...
                role_init=Path('infra/postgres/init/001-create-roles.sh').resolve()
                self.run(['run','-d','--name',primary,'--label',LABEL+'='+self.run_id,'--network',self.network,
                          ...
                          '-v',str(role_init)+':/docker-entrypoint-initdb.d/001-create-roles.sh:ro',IMAGE,'postgres',
                ...
                self.report['checks']['sameExactSchemaContract']=True
                ...
                markdown.extend(['','Exact command descriptions, durations, canonical digests, WAL/basebackup checksums, schema contract and controlled PITR boundary: ['+path.name+']('+path.name+').','',
