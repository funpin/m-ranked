...
-- A new database declares both constraints validated; here they are added NOT
-- VALID so this transaction never scans seven million rows under ACCESS
-- EXCLUSIVE. Every existing row already satisfies them, so the operator runs
-- VALIDATE CONSTRAINT straight after the release under SHARE UPDATE EXCLUSIVE,
-- which does not block collection. See operations/runbooks/CAPACITY.md.
ALTER TABLE ingest.publication_metric_snapshot
    ...
    ADD CONSTRAINT snapshot_evidence_dictionary_fk FOREIGN KEY (metric_evidence_id)
        REFERENCES ingest.metric_evidence_dictionary(id) NOT VALID;
...
CREATE OR REPLACE VIEW ops_and_admin.schema_contract AS SELECT 'storage-publisher-final-2026-09-08-r4'::text AS contract_id;
-- r4-delta:end
COMMIT;
